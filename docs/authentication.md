...
Senhas têm 12–128 caracteres e não são normalizadas. O hash é PBKDF2-HMAC-SHA-256 com salt aleatório de 128 bits, pepper secreto, 600.000 iterações e saída de 256 bits. Comparações usam operação constante fornecida pelo runtime. Contas inexistentes executam derivação falsa antes da resposta genérica.
...
Login rotaciona a sessão atual. Logout revoga uma; logout-all revoga todas. Contas disabled não autenticam. O token CSRF é independente, rotacionado na leitura da sessão e mantido somente em memória pelo frontend.

## Antiabuso

Limites combinam IP e conta/usuário em `SecurityGate`, sem usar IP bruto como nome do Durable Object. Falhas aumentam cooldown progressivo e acionam Turnstile adaptativo a partir do limiar configurado. Uma senha correta limpa o cooldown, evitando que tentativas de terceiros bloqueiem indefinidamente o titular.
